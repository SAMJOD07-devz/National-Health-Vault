...
import { mockMedications, mockVisits, mockAuditLogs } from '../data/mockDb';
...
export default function PatientDashboard() {
  ...
  const activeMeds = mockMedications.filter(m => m.patientId === user.id && m.status === 'active');
  const patientVisits = mockVisits.filter(v => v.patientId === user.id);
  const auditLogs = mockAuditLogs.filter(a => a.patientId === user.id);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 md:col-span-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium text-slate-800">{user.dob}</p>
              </div>
              <div>
                <p className="text-slate-500 mb-1">Gender</p>
                <p className="font-medium text-slate-800">{user.gender}</p>
              </div>
              <div>
                <p className="text-slate-500 mb-1">Aadhaar</p>
                <p className="font-medium text-slate-800">{user.aadhaarMasked}</p>
              </div>
              ...
              <div>
                ...
                <p className="font-medium text-slate-800">{user.emergencyContact}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-nhv-blue p-6 rounded-2xl shadow-sm text-white flex flex-col justify-center items-center text-center relative overflow-hidden">
            ...
            <p className="text-5xl font-bold">{user.bloodGroup}</p>
          </div>
        </div>
              ...
              {activeMeds.length > 0 ? activeMeds.map(med => (
                <div key={med.id} className="flex justify-between items-center p-3 bg-slate-50 rounded-lg border border-slate-100">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-slate-400 mt-1">Since {med.startDate}</p>
                  </div>
                </div>
              )) : (
                ...
                <div key={visit.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  ...
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white p-4 rounded-lg border border-slate-100 shadow-sm">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <time className="text-xs text-slate-400">{visit.date}</time>
                    </div>
                    <p className="text-sm text-slate-500 line-clamp-2">{visit.notes}</p>
                    {visit.reportFileUrl && (
                      <button className="mt-2 text-xs font-medium text-nhv-blue flex items-center gap-1 hover:underline">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <h2 className="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2">
            ...
            Access & Audit Log
            <span className="ml-auto text-xs font-normal text-slate-400">Builds Trust - Hackathon Feature</span>
          </h2>
            ...
            <table className="w-full text-sm text-left text-slate-500">
              <thead className="text-xs text-slate-700 uppercase bg-slate-50">
                <tr>
                  ...
                  <th className="px-4 py-3 rounded-r-lg">Action Taken</th>
                </tr>
              </thead>
              <tbody>
                {auditLogs.map(log => (
                  <tr key={log.id} className="border-b last:border-0 border-slate-100">
